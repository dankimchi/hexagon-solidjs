...
function App() {
			...
			if (combo().includes(cell.letter)) {
				...
				if (newCombo.length > 2) {
					if (calledCombos().includes(newCombo))
						showResponseToUser(gamePhases.alreadyCalled);
					// check if combo of 3 letters has correct sum
						...
						if (calledCombos().length == hexagon().rarity[sum()] - 1)
							...
							showResponseToUser(gamePhases.correct);
					} else showResponseToUser(gamePhases.wrong);
				}
			}
		}
	}
	...
	function showResponseToUser(phase: gamePhases) {
		setPhase(phase);
		setTimeout(() => {
